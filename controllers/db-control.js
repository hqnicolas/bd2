...
const triggersFilePath = path.join(__dirname, '..', '/src/scripts/triggers.sql');
const storedProceduresFilePath = path.join(__dirname, '..', '/src/scripts/stored_procedures.sql');
const dmlFilePath = path.join(__dirname, '..', '/src/scripts/dml.sql');
...
const addTrigger = async (req, res) => {
  ...
  try {
    ...
    logger.error(`Error creating trigger ${triggerName}: ${err.stack}`);
    res.status(500).json({ error: 'Erro do Servidor Interno' });
  }
};
...
const deleteTrigger = async (req, res) => {
  ...
  try {
    ...
    logger.error(`Error deleting trigger ${triggerName}: ${err.stack}`);
    res.status(500).json({ error: 'Erro do Servidor Interno' });
  }
};
